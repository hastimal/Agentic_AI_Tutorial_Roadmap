...
### 1. Setup Environment & ADK Installation
...
> [!IMPORTANT]
> **ADK Namespace Package Setup**: The Google Antigravity SDK is vendored inside the `google/antigravity` directory. 
> To ensure compatibility with other pip-installed Google packages (such as `google-genai`):
> 1. **Do not place an `__init__.py` file inside the root `google/` directory**. Doing so will make Python treat it as a regular package, blocking access to other libraries in the `google` namespace. Leave the `google` folder without `__init__.py` so it acts as a native PEP 420 namespace package.
> 2. **Local Harness Permissions**: Ensure that the `localharness` binary has executable permissions:
>    ```bash
...
#### Option C: Deploy directly via the ADK CLI (Simplest methodology)
You can deploy your agent to Cloud Run in one command using the Agent Development Kit CLI:
```bash
adk deploy cloud_run \
    --project=gcp-10-project \
    --region=us-central1 \
    --service_name=social-agent-service \
    social_agent
```
This command automatically handles package builds, Artifact Registry uploads, and environment setups for Cloud Run.
